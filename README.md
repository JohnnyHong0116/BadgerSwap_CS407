...
| **Nov 10** | Posting + Listing Enhancement | Enable posting items + simple local messaging | **Register Screen** + local user storage + Profile Screen | **Post Item Screen** (form + preview) | **Favorites Screen** + toggle state within profile screen (because role A did not work on profile screen, and my work is based on this screen, so I worked on it) | **Message List Screen** (search + threads) |
